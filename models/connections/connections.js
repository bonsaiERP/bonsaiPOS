...
define(["database"], function(database) {
  ...
  var lista = database.getTable(table_name, '\\views\\connectionERP', 2);
  var user = database.getTable("token", '\\views\\connectionERP', 2);
    ...
    $("#token").blur(function () {
      ...
      if (tokenIsHere("token", token))
        ...
        if (token !== "") {
          ...
          var list = [];
          list.push(new_token);
          database.putTable("token", list, '\\views\\connectionERP', 2);
