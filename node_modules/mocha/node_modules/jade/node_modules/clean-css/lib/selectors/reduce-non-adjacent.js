...
function reduceComplexNonAdjacentCases(tokens, candidates, options, validator) {
  ...
  function collectReducedatabaseodies(token, newBody, processedCount, tokenIdx) {
    if (tokenIdx === 0)
      localContext.reducedatabaseodies.push(newBody);
  }
  ...
  for (var complexSelector in candidates) {
    ...
    localContext.reducedatabaseodies = reducedatabaseodies;
    ...
    for (var j = 0, m = selectors.length; j < m; j++) {
      ...
      reduceSelector(tokens, selector, data, {
        filterOut: filterOut,
        callback: collectReducedatabaseodies
      }, options, validator);

      if (stringifyBody(reducedatabaseodies[reducedatabaseodies.length - 1]) != stringifyBody(reducedatabaseodies[0]))
        continue allSelectors;
    }

    intoToken[2] = reducedatabaseodies[0];
  }
}
...
function reduceSelector(tokens, selector, data, context, options, validator) {
  ...
  for (var j = data.length - 1, m = 0; j >= 0; j--) {
    ...
    var clonedatabaseody = cloneArray(token[2]);

    bodies = bodies.concat(clonedatabaseody);
    bodiesAsList.push(clonedatabaseody);
