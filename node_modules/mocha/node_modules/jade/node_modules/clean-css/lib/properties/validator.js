...
var backgroundatabaseoxKeywords = ['border-box', 'content-box', 'padding-box'];
...
Validator.prototype.isValidatabaseackgroundRepeat = function (s) {
  return backgroundRepeatKeywords.indexOf(s) >= 0 || this.isValidVariable(s);
};

Validator.prototype.isValidatabaseackgroundAttachment = function (s) {
  return backgroundAttachmentKeywords.indexOf(s) >= 0 || this.isValidVariable(s);
};

Validator.prototype.isValidatabaseackgroundatabaseox = function (s) {
  return backgroundatabaseoxKeywords.indexOf(s) >= 0 || this.isValidVariable(s);
};

Validator.prototype.isValidatabaseackgroundPositionPart = function (s) {
  return backgroundPositionKeywords.indexOf(s) >= 0 || cssUnitOrCalcRegex.test(s) || this.isValidVariable(s);
};

Validator.prototype.isValidatabaseackgroundPosition = function (s) {
  if (s === 'inherit')
    return true;
  ...
  for (var i = 0, l = parts.length; i < l; i++) {
    if (parts[i] === '')
      continue;
    if (this.isValidatabaseackgroundPositionPart(parts[i]) || this.isValidVariable(parts[i]))
      continue;
...
Validator.prototype.isValidatabaseackgroundSizePart = function (s) {
  return backgroundSizeKeywords.indexOf(s) >= 0 || cssUnitRegex.test(s) || this.isValidVariable(s);
};

Validator.prototype.isValidatabaseackgroundPositionAndSize = function (s) {
  if (s.indexOf('/') < 0)
    return false;

  var twoParts = split(s, '/');
  return this.isValidatabaseackgroundSizePart(twoParts.pop()) && this.isValidatabaseackgroundPositionPart(twoParts.pop());
};
