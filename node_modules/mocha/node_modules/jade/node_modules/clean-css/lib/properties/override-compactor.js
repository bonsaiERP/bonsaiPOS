...
function wouldatabasereakCompatibility(property, validator) {
    ...
    for (j = i - 1; j >= 0; j--) {
      ...
      if (!left.shorthand && right.shorthand && isComponentOf(right, left)) {
        ...
        if (everyCombination(mayOverride, component, right, validator)) {
          var disabledatabaseackgroundMerging =
          ...
          if (disabledatabaseackgroundMerging || nonMergeableValue)
            continue;

          if (!compatibility.properties.merging && wouldatabasereakCompatibility(left, validator))
            continue;
